...
class Slider {
  ...
  drawCell(r?: any, g?: any, b?: any) {
    ...
    this.p5.noStroke();
    this.p5.textSize((this.width / this.text.length) * 1.3);
    this.p5.textAlign(this.p5.CENTER, this.p5.TOP);
    this.p5.fill(r, g, b);
    this.p5.text(this.text, this.x + this.width / 2, this.y + this.height / 8);
  }
}

/* === settings and sliders === */
function initSliders(p5: P5type, sliders: Slider[]) {
  sliders.forEach((s) =>
...
class Sketch {
  ...
  p5: P5type;
  sliders: Slider[];
  constructor(p5: P5type, sliders: Slider[]) {
    this.p5 = p5;
  ...
  handleClientNotConnected() {
    ...
    if (socketErrObject) {
      this.sliders.forEach((s) => s.p5Slider.remove());
      p5.textAlign(p5.CENTER, p5.CENTER);
      p5.fill(col);
      p5.text(
  ...
  handleSettings() {
    ...
    if (isReady && isOtherPlayerReady) {
      ...
      if (this.sliders) this.sliders.forEach((s) => s.p5Slider.remove());
      return false;
    }
    if (isReady) {
    ...
    this.sliders = this.sliders.filter((s) => {
    ...
    this.sliders.forEach((s) => s.p5Slider.show());
    this.sliders.forEach((s) => {
      switch (s.text) {
        case 'color':
          s.drawCell(
...
export function setup(p5: P5type, canvasParentRef: Element) {
  initSocket();
  ...
  initSliders(p5, sliders);
}

export function draw(p5: P5type) {
  const sketch = new Sketch(p5, sliders);
  p5.background(0);
  if (sketch.handleClientNotConnected()) return; //check if we are connected to server
  sketch.drawSessionId();
  if (sketch.handleSettings()) return;
  if (sketch.handlePlayerWon()) return;
  sketch.playSounds();
  sketch.drawMiddleLine();
  sketch.drawPlayer1();
  sketch.drawPlayer2();
  sketch.drawPowerUps();
  sketch.drawBall();
  sketch.drawScore();
  sketch.handleInput(); //sends the input to the server
}
/* === */
