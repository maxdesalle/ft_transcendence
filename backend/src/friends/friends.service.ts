...
import {
  Friendship,
  FrienshipStatus as FriendshipStatus,
...
export class FriendsService {
  ...
  async sentRequests(my_id: number): Promise<any> {
    ...
    const requests = [];
    for (const rf of me.requested_friendships) {
      requests.push({
        recv_user: await this.usersRepository.findOneBy({
          id: rf.recv_user_id,
        }),
        status: rf.status,
      });
  ...
  async recvdRequests(my_id: number): Promise<any> {
    ...
    const requests = [];
    for (const rf of me.received_friendships) {
      requests.push({
  ...
  getUserStatus(user_id: number): string {
    if (playing.has(user_id)) return 'playing';
    if (this.wsService.isUserOnline(user_id)) return 'online';
    return 'offline';
  }
}
